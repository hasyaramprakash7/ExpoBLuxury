...
interface PricingBreakdown {
  ...
  gstAmount: number;
  finalTotal: number;
  // Assuming couponDiscount is part of the breakdown, although not defined here,
  // OrderSummary should be resilient to missing fields unless it needs to display them.
...
const OrderSummary: React.FC<OrderSummaryProps> = ({
  ...
  // Calculate total savings including coupon, if available
  const totalSavings =
  ...
  return (
    <View style={summaryStyles.container}>
      <View style={summaryStyles.card}>
        ...
        {pricingBreakdown.totalSavings > 0 && (
          <View style={summaryStyles.detailRow}>
            <Text style={summaryStyles.detailLabelSavings}>
              Discounts/Savings
            </Text>
        ...
        {(pricingBreakdown.couponDiscount || 0) > 0 && (
          <View style={summaryStyles.detailRow}>
            <Text style={summaryStyles.detailLabelSavings}>
              Coupon Discount
            </Text>
        ...
        <View style={summaryStyles.detailRow}>
          ...
          <Text style={summaryStyles.detailValueBold}>
            ₹
            {(
              pricingBreakdown.discountedSubtotal -
              (pricingBreakdown.couponDiscount || 0)
            ).toFixed(2)}
          </Text>
        </View>
        ...
        <View style={summaryStyles.paymentMethodSection}>
          ...
          <View style={summaryStyles.paymentOptionsContainer}>
            ...
            {/* Option 2: Cash on Delivery (COD) - RESTORED */}
            <TouchableOpacity
              style={[
                summaryStyles.paymentOption,
            ...
            >
              ...
              <View style={{ flex: 1, marginLeft: 15 }}>
                ...
                <Text style={summaryStyles.optionSubtitle}>
                  (Advance payment required in next step)
                </Text>
              </View>
              <Ionicons
                name="arrow-forward"
          ...
          <Text style={summaryStyles.paymentMethodHint}>
            {paymentMethod === "COD"
              ? "Final confirmation, address entry, and advance payment occur after clicking 'Place Order'."
...
const summaryStyles = StyleSheet.create({
  ...
  codOptionTitle: {
    color: Colors.rolexDarkText, // Retaining the main color
    fontWeight: "bold",
  },
  paymentMethodHint: {
    fontSize: 14,
