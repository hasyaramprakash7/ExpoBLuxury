...
export type RootStackParamList = {
  ...
  Chat: { forwardProduct?: Product }; // 🔥 Added Chat routing type
};
...
const Colors = {
  ...
  sizePillBorder: "#B0B0B0",
  sizeTextBlack: "#000000",
};
...
const NewProductCard: React.FC<NewProductCardProps> = ({
  ...
  const priceTiers = useMemo(() => {
    // ... [Your existing price tiers logic perfectly preserved] ...
    const tiers: any[] = [];
  ...
  const handleCartAction = async (qtyToDispatch: number) => {
    ...
    try {
      ...
      if (qtyToDispatch === 0)
        showToast(`Removed ${product.name}${sizeDisplay}`, "info");
      else if (!cartItem || cartItem.quantity === 0)
        showToast(
          `Added ${qtyToDispatch} x ${product.name}${sizeDisplay}`,
          "success",
        );
      else
        showToast(
          `Updated cart: ${qtyToDispatch} x ${product.name}${sizeDisplay}`,
          "info",
        );
    } catch (error: any) {
  ...
  const getButtonText = () => {
    ...
    if (isDisabled) return isVendorOffline ? "Vendor Offline" : "Out of Range";
    ...
    >
                  ...
                  {product.sizes?.map((size) => (
                    <TouchableOpacity
                      ...
                      style={[
                        ...
                        isDisabled && styles.sizePillDisabled,
                      ]}
        ...
        <View style={styles.bottomDetailsContainer}>
            ...
            {displayStock > 0 ? (
              displayStock <= 10 ? (
                <Text style={styles.limitedStockText}>
                  Limited! ({displayStock} Kg in stock)
                </Text>
              ) : (
          ...
          {isDisabled || displayStock === 0 ? (
            <View style={styles.disabledButton}>
              <Text style={styles.disabledButtonText}>{getButtonText()}</Text>
            </View>
            ...
            >
              ...
              <View style={styles.quantityButtonsWrapper}>
                ...
                <TextInput
                  ...
                  style={[styles.quantityInput, { color: Colors.greenDark }]}
      ...
      <View style={styles.imageContainer}>
        <View style={styles.imageWrapper}>
          ...
          {/* 🔥 NEW: SHARE TO CHAT BUTTON OVERLAY */}
          <TouchableOpacity
          ...
          <View style={styles.priceDisplay}>
            {product.discountedPrice &&
              product.discountedPrice < product.price && (
                <Text style={styles.originalPriceDisplayText}>
...
const styles = StyleSheet.create({
  ...
  bottomDetailsContainer: { flexGrow: 1, justifyContent: "flex-end" },
  stockContainer: {
    flexDirection: "row",
    alignItems: "center",
    gap: 2,
  ...
  limitedStockText: { fontSize: 8, color: Colors.redAlert, fontWeight: "600" },
  inStockContainer: { flexDirection: "row", alignItems: "center", gap: 2 },
  inStockText: { fontSize: 8, color: Colors.textLight, fontWeight: "600" },
  outOfStockContainer: { flexDirection: "row", alignItems: "center", gap: 2 },
  outOfStockText: { fontSize: 8, color: Colors.redAlert, fontWeight: "600" },
  priceTiersContainer: {
    flexDirection: "column",
    gap: 4,
    paddingVertical: 2,
  ...
  priceTierActive: { backgroundColor: Colors.greenDark },
  priceTierLabel: { fontSize: 8, fontWeight: "500" },
  priceTierLabelActive: { color: Colors.textLight },
  priceTierPrice: { fontSize: 12, fontWeight: "bold", color: Colors.textLight },
  priceTierPriceActive: { color: Colors.textLight },
  sizeSelectorWrapper: {
    flexDirection: "column",
    marginTop: 6,
    marginBottom: 6,
  ...
  selectedSizeText: { color: Colors.textLight, fontWeight: "900" },
  sizeScrollView: { alignItems: "center", paddingBottom: 2 },
  sizePill: {
    paddingHorizontal: 8,
    paddingVertical: 3,
    borderRadius: 12,
    borderWidth: 1,
    borderColor: Colors.sizePillBorder,
    marginHorizontal: 3,
  ...
  sizePillText: { fontSize: 9, color: Colors.sizeTextBlack, fontWeight: "600" },
  sizePillTextActive: { color: Colors.textLight },
  sizePillDisabled: { opacity: 0.5 },
  quantityControlsContainer: { flexDirection: "column", gap: 2, marginTop: 4 },
  addedToCartMessage: {
    flexDirection: "row",
    alignItems: "center",
  ...
  addToCartButtonDisabled: { backgroundColor: Colors.grayLight },
  addToCartButtonText: {
    fontSize: 10,
    fontWeight: "600",
    color: Colors.textLight,
  ...
  noImageText: { fontSize: 10, color: Colors.grayText, marginLeft: 2 },
  overlay: {
    position: "absolute",
    top: 0,
    left: 0,
  ...
  overlayDanger: { backgroundColor: "rgba(220, 38, 38, 0.6)" },
  overlayText: {
    color: Colors.textLight,
    fontWeight: "bold",
    fontSize: 12,
  ...
  saveBadgeText: { color: Colors.textDark, fontSize: 8, fontWeight: "bold" },
  priceDisplay: {
    position: "absolute",
    bottom: 2,
    left: 2,
